...
int main() {
    ...
    std::ifstream inputFile("C:/Users/user/Downloads/ESTRUCTURA/Experimental2/datos20.txt");
    std::string line;
    ...
    std::ofstream outputFile("C:/Users/user/Downloads/ESTRUCTURA/Experimental2/countRegion20.txt");

    if (outputFile.is_open()) {
        for (int j = 0; j < 5; ++j) {
            outputFile << "Region " << regionRadius[j] << "x" << regionRadius[j] << ":" << std::endl;
            for (int k = 0; k < 20; ++k) {
                std::cout << "Tiempo de ejecucion countRegion 20 datos con centro P(0,0) y area del cuadrado " << regionRadius[j] << "x" << regionRadius[j] << " (iteracion " << (k + 1) << "):" << endl;
        ...
        outputFile.close();
        cout << "Los tiempos se han guardado en el archivo countRegion20.txt" << endl;
    } else {
        cout << "No se pudo abrir el archivo countRegion20.txt" << endl;
    }
